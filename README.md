...
- **数据源**：腾讯行情接口 `qt.gtimg.cn`（免费、无需 key，与 PC 盯盘工具同源）
...
## 二、Sideloadly 安装到 iPhone

**一次性准备：**

1. 电脑安装 [Sideloadly](https://sideloadly.io/)（Windows 版）。
2. Windows 还需要 iTunes（或微软商店的「Apple Devices」应用）提供 USB 驱动。
3. 准备一个 **App 专用密码**：appleid.apple.com → 登录 → 登录与安全 → App 专用密码 → 生成（开启了两步验证的 Apple ID 必须用这个，不能用账号密码）。

**安装步骤：**

1. 数据线连接 iPhone（首次连接手机上点「信任」）。
2. 打开 Sideloadly，拖入 `StockIsland.ipa`。
3. Apple ID 填你的账号，点 Start，密码框填 **App 专用密码**。
4. 手机上：设置 → 通用 → **VPN与设备管理** → 找到你的 Apple ID → 点「信任」。
5. 打开「股票灵动岛」app → 首次点「开始盯盘」时弹窗允许「实时活动」。

**注意：免费 Apple ID 签名有效期 7 天**。到期后 app 打不开时，重复上面 1~4 步重签重装即可（数据保留，两三分钟的事）。

## 三、使用说明

- **股票代码**：美股直接字母（`AAPL`、`TSLA`）；A股 6 位数字（`600519`）；港股带 hk 前缀（`hk09988`）——与你 PC 盯盘工具同一套规则。
...
| 签名 7 天过期 app 闪退 | 连电脑用 Sideloadly 重签（见上文） |
| Sideloadly 报 anisette/provisioning 错误 | 多为网络问题，重试；或登录时换成 App 专用密码 |
...
- **GBK 解码**：腾讯接口返回 GBK，Swift 用 `CFStringEncoding(kCFStringEncodingGB_18030_2000)` 转码。
- **涨跌幅口径**：`(现价-昨收)/昨收`，与 PC 盯盘工具一致（美股以昨收为基准，盘中口径）。
